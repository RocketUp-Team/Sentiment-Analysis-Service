...
\title{\textbf{Sentiment Analysis Service}\\Gói bàn giao dự án end-to-end hoàn chỉnh}
\author{Tài liệu bàn giao nội bộ ưu tiên mã nguồn}
\date{2026-06-05}
...
{\large\bfseries Gói bàn giao dự án end-to-end hoàn chỉnh}\\[0.35em]
{\normalsize Phục vụ đánh giá CTO, thẩm định kỹ thuật, production readiness, bàn giao, và onboarding}\\[1.0cm]
...
Advanced MVP & \textbf{Trạng thái hiện tại}. Hệ thống có luồng end-to-end thực, có monitoring, có vòng đời training, nhưng còn thiếu các kiểm soát production. \\
Pre-Production & Đạt một phần. Có container hóa và cấu trúc vận hành, nhưng thiếu xác thực, endpoint trạng thái batch chỉ là mock, nhiều trạng thái nằm trong memory, và chưa có hệ thống job bền vững. \\
...
\begin{itemize}[leftmargin=1.2em]
  ...
  \item Các kiểm soát production quan trọng chưa có: xác thực, phân quyền, quản lý bí mật tốt, lưu trạng thái bền vững, giới hạn tốc độ, và job queue bền vững.
\end{itemize}
...
Feedback & Lưu phản hồi nghiệp vụ & \textbf{Chưa triển khai} & Solution & N/A & Chưa có vòng lặp phản hồi khép kín \\
...
\texttt{docs} & Tài liệu, handbook, báo cáo cũ & Thấp đối với runtime & Tech Lead + Solution & Trung bình vì có thể gây hiểu nhầm nếu lệch code \\
...
Lỗi hiện được xử lý ở mức route và runtime, nhưng chưa có phân loại lỗi đủ mạnh, chưa có luồng thất bại cho xác thực, và chưa có cơ chế kiểm soát job idempotent. Về vận hành, điều này có nghĩa là sự cố sẽ thường phải đọc log và tái hiện thủ công nhiều hơn.

\subsection{Khoảng trống backend}
\begin{itemize}[leftmargin=1.2em]
  \item Chưa triển khai: authentication
  \item Chưa triển khai: authorization
  \item Chưa triển khai: durable batch job system
  \item Chưa triển khai: persistent request/job state store
  \item Triển khai một phần: đồng bộ contract giữa docs, frontend, và runtime
\end{itemize}
...
\midrule
Không có xác thực cho API & Critical & Backend + Solution & thêm authn, token/session policy, bảo vệ admin endpoints \\
Không có phân quyền cho evaluate/admin-like actions & Critical & Backend + Solution & tách admin surface và policy kiểm soát quyền \\
...
\midrule
Critical & Không có xác thực và phân quyền & không thể mở production an toàn & Backend + Solution & thêm auth, tách admin surface, kiểm soát quyền \\
...
Low & README và contract docs cũ hơn implementation & onboarding và thẩm định kỹ thuật chậm hơn & Tech Lead & cập nhật tài liệu theo code \\
\bottomrule
\end{longtable}
