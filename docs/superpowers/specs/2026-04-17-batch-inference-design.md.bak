...
| `tests/model/test_baseline.py` | Add chunking + skip_absa unit tests |
| `tests/model/test_batch_throughput.py` | **NEW** — throughput benchmark |

### 6. Files NOT Changed

- `src/main.py` — endpoint logic untouched
- `src/model/evaluate.py` — training evaluation untouched
...
| Test | Target |
|---|---|
| `test_sentiment_throughput_meets_target` | skip_absa=True, 200 texts → ≥ 100 samples/sec |
